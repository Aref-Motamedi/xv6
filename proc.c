  ...
  for(;;){
   if (policy==2){
      ...
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        ...
        if (bestpriority==-1){
          bestpriority = p->calculatedPriority;
        } else if (bestpriority > p->calculatedPriority){
          bestpriority = p->calculatedPriority;
        }
      }
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        if(p->state != RUNNABLE)
          continue;
        if (p->calculatedPriority != bestpriority){
          continue;
        }
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        ...
        timevar->creationTime = p->creationTime;
        timevar->readyTime = p->readyTime;
